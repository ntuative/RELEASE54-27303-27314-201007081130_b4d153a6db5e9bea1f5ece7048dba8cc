package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1641 == RoomWidgetRoomQueueUpdateEvent.const_343)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_621);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1641 = param1.type;
            var_1123 = false;
            var_1122 = param1.position;
         }
         else
         {
            var_1123 = true;
         }
         var_2136 = param1.hasHabboClub;
         var_1640 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",var_1122.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_1122.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_1122.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_1122.toString());
         showInterface();
      }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_343,onQueueStatus);
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_499,onQueueStatus);
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_573);
         messageListener.processWidgetMessage(_loc2_);
         removeWindow();
      }
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_343,onQueueStatus);
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_499,onQueueStatus);
      }
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_681));
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1641)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_343:
                  _loc1_.caption = !!var_1640 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_1123;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_499:
                  _loc1_.caption = !!var_1640 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
