package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_1756:int = 100;
      
      private const const_1453:int = 1;
      
      private var var_330:AssetLoaderStruct;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         var_330 = _assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         addMapLoaderEventListeners();
         _downloadShiftTimer = new Timer(const_1756,1);
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in var_798)
         {
            _loc7_ = true;
            _loc4_ = var_798[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete var_713[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete var_798[_loc3_];
         }
         while(_loc6_ < var_986.length)
         {
            _loc5_ = var_986[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               var_986.splice(_loc6_,1);
            }
            _loc6_++;
         }
      ...
      public function loadFigureSetData(param1:AvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = var_713[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               var_713[_loc3_] = _loc6_;
            }
            var_798[_loc3_] = _loc4_;
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + var_988);
            if(var_1420.indexOf("?") > 0)
            {
               _loc2_ = var_1420 + "&retry=" + var_988;
            }
            else
            {
               _loc2_ = var_1420 + "?retry=" + var_988;
            ...
            var_330 = _assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            addMapLoaderEventListeners();
         }
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(var_330)
         {
            var_330.addEventListener(AssetLoaderEvent.const_29,onConfigurationComplete);
            var_330.addEventListener(AssetLoaderEvent.const_42,onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(var_330)
         {
            var_330.removeEventListener(AssetLoaderEvent.const_29,onConfigurationComplete);
            var_330.removeEventListener(AssetLoaderEvent.const_42,onConfigurationError);
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,var_2272,_assets);
            _loc3_.addEventListener(Event.COMPLETE,libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = var_987[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               var_987[_loc5_] = _loc6_;
            }
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(false && var_986.length < const_1453)
         {
            _loc1_ = var_673[0];
            _loc1_.startDownloading();
            var_986.push(var_673.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(var_673.indexOf(param1) == -1 && !param1.isReady)
         {
            var_673.push(param1);
            processPending();
         }
      ...
      private function onConfigurationComplete(param1:Event) : void
      {
         ...
         generateMap(data);
         var_1680 = true;
         for each(initListener in var_799)
         {
