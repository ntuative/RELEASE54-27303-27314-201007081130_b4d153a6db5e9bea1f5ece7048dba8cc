package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(var_423.getValue(_loc4_) == null)
               {
                  var_423.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
         ...
         for each(_loc6_ in _loc4_)
         {
            if(var_1086.indexOf(_loc6_) == -1)
            {
               ...
               if(var_1085.indexOf(_loc6_) == -1)
               {
                  ...
                  if(param3 != null && var_713.indexOf(param3) == -1)
                  {
                     var_713.push(param3);
                  }
               }
      ...
      {
         var _loc3_:IPetData = var_423.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      private function notifyListeners() : void
      {
         ...
         while(false)
         {
            _loc1_ = var_713.pop() as IPetDataListener;
            if(_loc1_ != null)
            {
               _loc1_.petDataReady();
            }
      ...
      {
         var _loc4_:PetData = var_423.getValue(param1);
         if(_loc4_ == null)
         {
            Logger.log("Could not find Pet Data: " + param1);
            return "";
         }
         var _loc5_:Breed = _loc4_.method_16(param2);
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = var_423.getKeys();
         for each(key in speciesKeys)
         {
            petData = var_423.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.method_5.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
