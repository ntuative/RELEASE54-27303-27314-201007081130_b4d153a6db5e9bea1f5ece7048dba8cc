package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_443 >= var_323.length)
         {
            return "";
         }
         var petColor:IPetColor = var_323[var_443] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_651[var_1642] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_443 = param1.index;
         if(var_443 < 0 || var_443 > var_323.length)
         {
            var_443 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_665.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_117,onTextWindowEvent);
         var_665 = new Map();
         ...
         var_950 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_323 = [];
         var_651 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_950,this);
               if(_loc11_)
               {
                  var_651 = _loc11_.breeds;
                  var_323 = _loc11_.colors;
                  if(var_323 == null)
                  {
                     var_323 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_651.length)
         {
            _loc12_ = var_651[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_950,_loc12_.id),getRaceLocalizationKey(var_950,_loc12_.id)));
            _loc6_++;
         }
         var_665.add(var_950,_loc5_);
         ...
         _loc5_ = var_665.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_323)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_665.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_443 >= 0 && var_443 < var_323.length)
         {
            _loc7_ = var_323[var_443] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_651[var_1642] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(var_950,_loc3_.id,_loc2_,AvatarScaleType.const_49,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_281,AvatarAction.const_752);
